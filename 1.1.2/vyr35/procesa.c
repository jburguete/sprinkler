...
main ()
{
  double xm[NX] = { 1.5, 3., 4.5, 6., 7.5, 9., 10.5, 12. };
  double ym[NY] = { -1.35, 0., 0.9, 1.4 };
  double dm[ND] = { 0., 1., 2., 3., 4. };
  double vx[NT], vy[NT];
  double v, angle, x[5];
  ...
  filein = fopen ("DatosGotas.txt", "r");
  if (!filein)
    {
      show_error ("Unable to open the DatosGotas.txt file\n");
      return 1;
    }
  while (fscanf (filein, "%lf%lf%lf%lf%lf", x, x + 1, x + 2, x + 3, x + 4) == 5)
    {
      for (i = 0; i < NX; ++i)
        if (x[1] == xm[i])
          break;
